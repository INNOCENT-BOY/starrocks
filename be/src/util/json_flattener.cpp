...
void JsonPathDeriver::init_flat_json_config(const FlatJsonConfig* flat_json_config) {
    if (flat_json_config != nullptr) {
        _max_json_null_factor = flat_json_config->getFlatJsonNullFactor();
        _min_json_sparsity_factory = flat_json_config->getFlatJsonSparsityFactor();
        _max_json_null_factor = flat_json_config->getFlatJsonMaxColumnMax();
    } else {
