...
Status SegmentWriter::init(const std::vector<uint32_t>& column_indexes, bool has_key, SegmentFooterPB* footer) {
    ...
    for (uint32_t i = 0; i < _column_indexes.size(); i++) {
        ...
        if (column.type() == LogicalType::TYPE_JSON) {
            if (_opts.flat_Json_config != nullptr) {
                opts.need_flat = _opts.flat_Json_config->isFlatJsonEnabled();
                opts.meta->mutable_json_meta()->set_is_flat(_opts.flat_Json_config->isFlatJsonEnabled());
                ...
                opts.meta->mutable_json_meta()->set_is_flat(config::enable_json_flat);
                opts.meta->mutable_json_meta()->set_flat_json_null_factor(config::json_flat_null_factor);
                opts.meta->mutable_json_meta()->set_flat_json_sparsity_factory(config::json_flat_sparsity_factor);
                opts.meta->mutable_json_meta()->set_flat_json_column_max(config::json_flat_column_max);
            }
        }
